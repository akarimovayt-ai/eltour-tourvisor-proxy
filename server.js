...
// Найти код города вылета по названию
...
app.listen(PORT, function() { console.log('Proxy запущен на порту ' + PORT); });async function fetchTourvisorJSON(url) 
  var r = await fetch(url);
  var buf = await r.arrayBuffer();
  try {
    var text = new TextDecoder('utf-8').decode(buf);
